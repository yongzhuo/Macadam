...
def metrics_report(y_true: List, y_pred: List, rounded: int=2, epsilon:float=1e-9, use_draw: bool=True):
    """
    ...
    if use_draw:
        ...
        for li in labels:
            ...
            report += row_fmt.format(li, p, r, f1, a, int(s), width=width, rounded=rounded)
        report += "\n"
        # 评估指标sum, mean
        for sm in sign_tol:
            [p, r, f1, a, s] = [metrics[sm][sm + "_" + hd] for hd in headers]
            report += row_fmt.format(sm, p, r, f1, a, int(s), width=width, rounded=rounded)
        report += "\n"
        # logger.info(report)
...
if __name__ == "__main__":
    ...
    # metrics report
    y_true = ["yes", "no", "pos", "yes", "no", "pos", "yes", "no", "pos", "yes", "no", "pos"]
    y_pred = ["yes", "yes", "pos", "no", "no", "no", "yes", "no", "pos", "yes", "no", "pos"]
